"""
speedUpItem: ロジスティック回帰で不具合発見の有無を予測し、
適合率・再現率・F値・正解率を表示する。
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ロジスティック回帰で不具合発見を予測")
    parser.add_argument(
        "--target",
    ...
    print_confusion_matrix(y_test, y_pred, title=f"ロジスティック回帰（目的: {target_label}）- 混同行列")
    metrics = calculate_binary_metrics(y_test, y_pred, y_pred_proba=y_pred_proba)
    print_binary_metrics(metrics, title=f"ロジスティック回帰（目的: {target_label}）- 評価指標")
