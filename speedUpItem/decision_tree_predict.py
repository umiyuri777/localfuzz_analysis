...
def main() -> None:
    parser = argparse.ArgumentParser(description="決定木で不具合発見を予測")
    parser.add_argument(
        "--target",
    ...
    model = build_decision_tree_pipeline(include_tree=False, random_state=RANDOM_STATE)
    model.fit(X_train, y_train)

    y_pred = model.predict(X_test)
    y_pred_proba = model.predict_proba(X_test)[:, 1]

    print_confusion_matrix(y_test, y_pred, title=f"決定木（目的: {target_label}）- 混同行列")
    metrics = calculate_binary_metrics(y_test, y_pred, y_pred_proba=y_pred_proba)
    print_binary_metrics(metrics, title=f"決定木（目的: {target_label}）- 評価指標")
